...
TEST(operator, plus) {
  ...
  S21Matrix given_2(size + 1, size + 1);
  EXPECT_THROW(given_2 + given_1, std::logic_error);
}
...
TEST(operator, mul) {
  ...
  S21Matrix m4 = m1 * m2;
  S21Matrix given_2(4, 4);
  EXPECT_THROW(given_2 * m1, std::logic_error);
  S21Matrix m5(m1);
  m1.MulMatrix(m2);
  ...
  S21Matrix m3(3, 1);
  EXPECT_THROW(m3.MulMatrix(m2), std::logic_error);
...
TEST(errors, get_minor_out_of_range) {
  S21Matrix A(3, 3);
  EXPECT_THROW(A.GetMinorMatrix(-1, -1), std::out_of_range);
}

TEST(errors, get_minor_not_square) {
  S21Matrix A(2, 3);
  EXPECT_THROW(A.GetMinorMatrix(1, 2), std::logic_error);
}
